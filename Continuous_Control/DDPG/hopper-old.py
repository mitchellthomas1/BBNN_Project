...
#                time_axis = np.array([time.time()-initial_time])
                temp_reward = 0
            temp_reward += ep_reward

            if i == 13:
                avg_time_axis = np.array([time.time() - initial_time])
            elif i == 25:
                avg_reward_axis = np.array([temp_reward/26])
                temp_reward = 0
            elif i % 13 == 0 and i !=0 and i != 13:
                time_midpoint = time.time() - initial_time
                np.append(avg_time_axis, time_midpoint)
            elif i % 25 == 0 and i !=0 and i != 25:
                print(temp_reward/25)
                print(time_midpoint)
                np.append(avg_reward_axis, temp_reward/25)
                temp_reward = 0
                ax1.plot(avg_time_axis, avg_reward_axis)
                ax1.grid()
                plt.show()
...
if __name__ == '__main__':
    trainer(epochs=1000, epsilon = .95, render = True)
